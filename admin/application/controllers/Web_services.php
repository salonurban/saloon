...
public function getstafflist_post()
{
	...
	if(isset($post_data['serviceid'])) {
		foreach($post_data['serviceid'] as $service_details) {
	...
	$post['staff_details'] = $this->staff_model->get_joint_staff($where);
	$this->response( $post );
...
public function getsingleshop_post(){
				...
				if($post_data['service_name']){
					$where = array('staffs_specialist.service_id' => $post_data['service_name'] );
				}
				else{
					$where=null;
				}
